...
public class Phishing extends LessonAdapter
{
    ...
    private boolean postedCredentials(WebSession s)
    {
        ...
        if (!postedToCookieCatcher.equals(Catcher.EMPTY_STRING)) {
            String postedUser = getLessonTracker(s).getLessonProperties().getProperty(
                    "phishingUsername", Catcher.EMPTY_STRING);
            String postedPassword = getLessonTracker(s).getLessonProperties().getProperty(
                    "phishingPassword", Catcher.EMPTY_STRING);
            System.out.println("ECE568 Part1: posted credentials: phishingUsername = " + postedUser + "; phishingPassword = " + postedPassword);
    ...
    public void restartLesson(WebSession s) {
        ...
        getLessonTracker(s).getLessonProperties().setProperty("phishingUsername", Catcher.EMPTY_STRING);
        getLessonTracker(s).getLessonProperties().setProperty("phishingPassword", Catcher.EMPTY_STRING);
    }
